...
const KNOBS = {
  a: { key: 'a', label: 'stretch / flip', color: C.orange, min: -3, max: 3, step: 0.25, init: 1 },
  h: { key: 'h', label: 'slide sideways', color: C.blue, min: -6, max: 6, step: 0.5, init: 0 },
  k: { key: 'k', label: 'slide up / down', color: C.green, min: -5, max: 5, step: 0.5, init: 0 },
};

const LEVELS = [
  { shape: 'line', controls: ['k'], fixed: { a: 0.5, h: 0 }, target: { k: 3 },
    hint: 'One green knob. Ride the elevator up to the ghost.' },
  { shape: 'abs', controls: ['h'], fixed: { a: 1, k: -2 }, target: { h: -4 },
    hint: 'Blue knob only. Remember: inside the parentheses it’s opposite-day.' },
  { shape: 'abs', controls: ['h', 'k'], fixed: { a: 1 }, target: { h: 3, k: 2 },
    hint: 'Two knobs. Park the corner of the V on the ghost’s corner.' },
  { shape: 'quad', controls: ['a', 'k'], fixed: { h: 0 }, target: { a: 0.5, k: -3 },
    hint: 'Make it lazier or taller, then lift or lower it.' },
  { shape: 'quad', controls: ['a', 'h'], fixed: { k: 1 }, target: { a: -1, h: 2 },
    hint: 'Somersault time. One of these knobs needs to go negative.' },
  { shape: 'wave', controls: ['a', 'h', 'k'], fixed: {}, target: { a: 2, h: 3, k: -1 },
    hint: 'Final boss: all three knobs. Sideways first, then height, then stretch.' },
];
    ...
    widgetMount.querySelector('.widget').insertAdjacentHTML('beforeend', `
      <div class="win-banner">
        <div class="win-text">${last
          ? '🏆 All six! You’re officially a graph wizard — every equation is just knobs now.'
          : '⭐ Nailed it!'}</div>
